...
CREATE TABLE `inventoryMovements` (
	...
	`movementType` enum('receiving','put_away','picking','transfer','adjustment','return','disposal') NOT NULL,
...
CREATE TABLE `pickingOrderItems` (
	...
	`serialNumber` varchar(100),
	`fromLocationId` int,
	`status` enum('pending','picked','short_picked','cancelled') NOT NULL DEFAULT 'pending',
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `pickingOrders` (
	...
	`status` enum('pending','in_progress','picked','checked','shipped','cancelled') NOT NULL DEFAULT 'pending',
	`assignedTo` int,
	`pickedBy` int,
	`pickedAt` timestamp,
	`createdBy` int NOT NULL,
...
CREATE TABLE `warehouseLocations` (
	...
	`status` enum('available','occupied','blocked','counting') NOT NULL DEFAULT 'available',
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
